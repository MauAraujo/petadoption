...
export async function updatePublication(publicationID, data) {
  const response = await axios
    .put(api + `/publications/${publicationID}`, data)
    .catch((err) => {
      console.log(err);
    });
  data.id = response.data.InsertedID;
...
export async function getRecommendations(animal, age, breed) {
  ...
  if (animal === "dog") {
    ...
    let response = await axios
      .get(api + `/recommendations/${age}/${breed}`)
      .catch((err) => {
        console.log(err);
      });
    ...
    let r = await axios.get(
      `https://api.thecatapi.com/v1/breeds/search?q=${breed}&api_key=${catApiKey}`
    );
    let id = r.data[0].id;
    let img = await axios.get(
      `https://api.thecatapi.com/v1/images/search?breed_id=${id}&api_key=${catApiKey}`
    );
    image = img.data[0].url;
    let response = await axios
      .get(api + `/recommendations/${age}/${id}`)
      .catch((err) => {
        console.log(err);
      });
...
function getDogRecommendations(age, dog) {
  ...
  if (dog["brachycephalic"]) recommendations.push(brachycephalic_nutrition);
  if (dog["sensitive_stomach"]) recommendations.push(dental_disease_nutrition);
  if (dog["obesity_prone"]) recommendations.push(obesity_nutrition);
  if (dog["dental_disease_prone"])
    recommendations.push(dental_disease_nutrition);
  if (dog["skin_problem_prone"]) recommendations.push(skin_problem_nutrition);
...
function getCatRecommendations(age, cat) {
  ...
  if (age === "baby") recommendations.push(kitten_nutrition);
  else if (age === "adult") recommendations.push(adult_cat_nutrition);
  else if (age === "senior") recommendations.push(senior_cat_nutrition);

  if (cat["recommendation"]) recommendations.push(cat["recommendation"]);
  else recommendations.push(cat_general_nutrition);
